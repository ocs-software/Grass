    ...
    try {
        ...
        if (account.length > 0) {
            if (account[0].token == token) {
                ...
                if (
                    account[0].linked_from != "" &&
                    account[0].linked_from != null
                ) {
        ...
        if (item.length > 0) {
            ...
            if (item[0].token == token) {
                ...
                if (sub_acc != "") {
                    ...
                    if (sub_accs.length > 0) {
                        if (sub_accs[0].linked_from != email) {
                            return await sendError(res, 200, {
                                thisDb,
                                errMess:
                                    "Only the Owner can delete this Sub-Account",
                                type: "validation",
                                action: "users/delete",
                                payload: req.query,
                                functionName: "users/delete",
                            ...
                            username =
                                sub_accs[0].user_firstname +
                                " " +
                                sub_acc[0].user_surname;
                            await thisDb.collection(table).deleteOne(query);
                            let res_json = { status: "OK" };

                            res_json.message =
                                "Sub-Account Deleted: " + sub_acc;
                            ...
                            templatemodel = {
                                ...
                                account_number: sub_acc,
                                important_00: "Account Deleted",
                                info: [{ infol: message }]
                            };
                            ...
                            // send email to Owner
                            message =
                                "A Sub-account you created , has been deleted. Details above.";
                            templatemodel = {
                                username: username,
                                subject: "Sub-Account Deleted",
                                account_number: sub_acc,
                                important_00: "Sub-Account Deleted",
                                info: [{ infol: message }]
                            };
                    ...
                    // if (!superToken) {
                    message =
                        "Your Account has been deleted , with any Sub-accounts , Venues or Courses you may of created.";
                    templatemodel = {
                        username: ownername,
                        subject: "Account Deleted",
    ...
    try {
        ...
        if (item.length > 0) {
            if (item[0].token == user_token) {
                ...
                query = { user_email: email };
                await thisDb.collection(table).updateOne(query, newvalues);

                const username =
                    item[0].user_firstname + " " + item[0].user_surname;
                const userurl = user_token + "~L~";
                const templatemodel = {
                    user_email: userurl,
                    booking: "Verify Account",
                    username: username,
        ...
        if (errMess !== "") {
            ...
            if (item.length > 0) {
                if (linked_email != "") {
                    ...
                    templatemodel = {
                        username: username,
                        subject: "Sub-Account Exists",
                        account_number: email,
                        important_00: "Sub-Account Exists",
                        info: [{ infol: message }]
                    };
                    ...
                    message =
                        "Your email has been used to try to Register a New Account in the App. If this is NOT you , please ignore this Email.";
                    templatemodel = {
                        user_email: userurl,
                        booking: "Re-Verify Account",
                        username: username,
                        subject: "Re-Verify Account",
                        info: [{ infol: message }]
                    };
                ...
                if (linked_email != "") {
                    ...
                    message =
                        "A new Sub-account has been created for: " + username;
                    templatemodel = {
                        username: username,
                        subject: "Sub-Account Created",
                        account_number: email,
                        important_00: "Sub-Account Created",
                        info: [{ infol: message }]
                    };
                ...
                if (linked_email != "") {
                    message =
                        linked_email +
                        " has created a new Sub-account for you.";
                    templatemodel = {
                        user_email: userurl,
                        booking: "Verify New Account",
                        username: username,
...
router.post("/update", async (req, res) => {
    ...
    try {
        ...
        if (errMess !== "") {
            ...
            if (token == process.env.TOKEN) superToken = true;
...
router.post("/golfbag", async (req, res) => {
    ...
    try {
        const { user_email, golf_bag, token } = req.body;
...
router.post("/deleteTour", async (req, res) => {
    ...
    try {
        ...
        if (errMess !== "") {
            ...
            res.status(200).send({
                status: "OK",
                message: resp ? "Record deleted." : "No record found to delete."
            });
        }
    } catch (e) {
        return await sendError(res, 400, {
...
router.post("/import", async (req, res) => {
    ...
    try {
        ...
        if (data?.players) {
            ...
            res.status(200).send({
                status: "OK",
                processed: response?.pcount,
                failed: response?.fcount,
    ...
    async function processData(data, res, thisDb, query, table) {
        ...
        if (errMess !== "") {
            ...
            if (Object.keys(setFields).length === 0) {
                if (
                    tour_obj &&
                    typeof tour_obj === "object" &&
                    !Array.isArray(tour_obj) &&
                    Object.keys(tour_obj).length > 0
                ) {
                } else {
                    res.fcount++;
                ...
                if (result.matchedCount === 0 && !result.upsertedId) {
                    ...
                    user_changes =
                        result.modifiedCount > 0 || result.upsertedId;
                }
            }
            ...
            if (Object.keys(tour_obj).length > 0) {
                ...
                const new_data = [
                    {
                        $set: {
                            ...
                            history: {
                                $cond: [
                                    // create history of member code if already exist one and code has changed
                                    {
                                        $and: [
                                            {
                                                $ne: [
                ...
                result = await toursDb.updateOne(query, new_data, {
                    upsert: true
                });
    ...
    function endImport(
        ...
        Promise.resolve()
            .then(async () => {
                const user_email = (
                    old_user_obj?.user_email ?? user_obj?.user_email
                )
                    .trim()
                    .toLowerCase();
                ...
                const _id =
                    old_user_obj?._id ??
                    (await getUserId(user_email, thisDb, table));
