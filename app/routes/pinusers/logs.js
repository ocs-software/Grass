...
router.get("/venue", async (req, res) => {
    try {
        ...
        if (errMess !== "") {
            ...
            let query = { venue: venue };
            // thisDb.collection("logs").find(query).toArray(function (err, logs) {
            ...
            thisDb.collection("logs").find(query).sort(mysort).toArray(function (err, logs) {
                if (err) {
                    ...
                    res_json.message = "Fetching Logs For Venue";
                    res_json.venue = venue;
                    res.send({ res_json });
                } else {
                    if (logs.length > 0) {
                        let res_json = { status: "OK", }
                        res_json.message = "Logs For Venue Found";
                        res_json.venue = venue;
                        res_json.logs = logs;
        ...
        res_json.message = "Error in getting logs for Venue.";
        res.res_json = res_json;
        res_json.venue = venue;
...
router.get("/course", async (req, res) => {
    try {
        ...
        if (errMess !== "") {
            ...
            thisDb.collection("logs").find(query).sort(mysort).toArray(function (err, logs) {
                if (err) {
                    console.log(err)
                    let res_json = { status: "FAILED", };
                    res_json.message = "Fetching Logs For Course";
                    res_json.course = course;
                    res.send({ res_json });
                } else {
                    if (logs.length > 0) {
                        let res_json = { status: "OK", }
                        res_json.message = "Logs For Course Found";
                        res_json.course = course;
                        res_json.logs = logs;
                        res.res_json = res_json;
                        res.send({ res_json });
        ...
        res_json.message = "Error in getting logs for Course.";
        res.res_json = res_json;
        res_json.course = course;
        res.status(400).send({ message: res_json.message, data: e });
    }
